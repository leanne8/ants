...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        #if ant is in path, block it. if it is ninjaant let it pass
        if self.place.ant:
            if self.place.ant.blocks_path == False:
                return self.place.ant is None
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while start != hive:
            if len(start.bees) != 0 and total>=self.min_range and total <= self.max_range:
...
class LongThrower(ThrowerAnt):
    ...
    name = 'Long'
    food_cost = 3
    min_range = 4 # 4 entrance transitions.
    implemented = True
...
class ShortThrower(ThrowerAnt):
    ...
    name = 'Short'
    food_cost = 3
    max_range = 2 # 2 entrance transitions.
    implemented = True
...
class QueenPlace:
    ...
    def __init__(self, colony_queen, ant_queen):
        "*** YOUR CODE HERE ***"
        if type(colony_queen) == QueenPlace:
            self.colony_queen = colony_queen.colony_queen
        else:
    ...
    def action(self, colony):
        ...
        if self.place:
            ...
            if self.imposter:
                ...
                if self.place:
                    ant_place = self.place.entrance
                    self.buff(ant_place, self.moveForward)
                    self.buff(backward, self.moveBackward)
                    while ant_place:
                        if ant_place.ant and (ant_place.ant != self) and (ant_place.ant not in self.double_damage):
                            ant_place.ant.damage *= 2
                            self.double_damage.append(ant_place.ant)
                        ant_place = ant_place.exit

    def damage2(self, ant):
        if (ant!= None and ant not in self.double_damage):
            ant.damage *= 2
            self.double_damage.append(ant)
        if(type(ant) is BodyguardAnt):
            self.damage2(ant.ant)

    def buff(self, ant_place, backward):
        while(ant_place != None):
            self.damage2(ant_place.ant)
            ant_place= backward(ant_place)

    def moveForward(self, place):
        return place.entrance

    def moveBackward(self, place):
        return place.exit
